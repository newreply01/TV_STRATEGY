...
export default function ScriptsListClient({ initialScripts }: { initialScripts: any[] }) {
  ...
  const [statusFilter, setStatusFilter] = useState('All'); // 新增實作狀態篩選
  const [search, setSearch] = useState('');

  const filteredScripts = initialScripts.filter(s => {
    const matchesCategory = filter === 'All' || s.category === filter;
    const matchesStatus = statusFilter === 'All' || 
                          (statusFilter === 'Implemented' && s.is_implemented) ||
                          (statusFilter === 'Planned' && !s.is_implemented);
      ...
      <header className="max-w-7xl mx-auto mb-12 flex flex-col lg:flex-row lg:items-end justify-between gap-8">
        <div className="space-y-4">
          ...
          <p className="text-zinc-500 max-w-2xl font-medium">
             為您精選 TradingView 頂尖指標，並透過 Python 引擎重現專業級動態分析與回測。
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4">
           ...
           <Link href="/admin/implementations" className="flex items-center gap-2 px-6 py-3 rounded-2xl bg-zinc-900 text-white border border-white/10 hover:bg-zinc-800 transition-all text-sm font-bold">
             實作進度面版
           </Link>
        </div>
      </header>
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="flex flex-wrap items-center gap-3">
             <div className="flex items-center gap-2 text-zinc-500 mr-2 border-r border-zinc-200 dark:border-white/10 pr-4">
                ...
                <span className="text-[10px] font-black uppercase tracking-widest">文章類別</span>
             </div>
             {CATEGORIES.map((cat) => (
               <button
                 key={cat}
          ...
          <div className="flex flex-wrap items-center gap-3">
             <div className="flex items-center gap-2 text-zinc-500 mr-2 border-r border-zinc-200 dark:border-white/10 pr-4">
                ...
                <span className="text-[10px] font-black uppercase tracking-widest">實作狀態</span>
             </div>
             {[
               { id: 'All', label: '全部' },
               { id: 'Implemented', label: '已實作完成' },
               { id: 'Planned', label: '規劃中/未實作' }
             ].map((status) => (
               <button
                 key={status.id}
