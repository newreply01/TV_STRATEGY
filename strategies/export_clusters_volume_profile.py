...
<head>
    ...
    <style>
        ...
        .v-bar {
            height: 2px; /* Adjusted for thinner bars */
...
<body>
    ...
    <script>
        ...
        Object.entries(groups).forEach(([color, members]) => {
            ...
            marker.style.padding = '0 80px 0 10px';
            
            marker.innerHTML = `
                <div class="v-label" style="color: ${color}; border: none; background: transparent; padding: 4px;">${(totalVol/1000).toFixed(1)}K</div>
                <div class="v-line" style="background-image: linear-gradient(to right, ${color} 50%, transparent 50%); background-size: 3px 1px; background-repeat: repeat-x; height: 1px; flex: 1; opacity: 0.9;"></div>
